...
from tensorflow.python.keras.layers import Conv1D, MaxPooling1D, Add, Flatten
...
class VGG1Dv3():
    ...
    def model_init(self, input_shape=(config.SAMPLE_RATE, 1)):
        ...
        def Conv1Dbn(x, conv_count, ks, ch, dropout=None, st=1):
        ...
        x = Conv1Dbn(x, 2, 3, 128, dropout=0.1)
...
class MelSpectCNN():
    ...
    def model_init(self, input_shape=(1, 128, 32)):
        
        x_in = Input(shape=input_shape)
        x = BatchNormalization()(x_in)
        x = Conv2D(16*(2 ** 0), (3, 3), data_format="channels_first")(x)
        x = Activation('relu')(x)
        x = BatchNormalization()(x)
        x = Conv2D(16*(2 ** 0), (3, 3), data_format="channels_first")(x)
        x = Activation('relu')(x)
        ...
        x_branch_1 = GlobalAveragePooling2D(data_format="channels_first")(x)
        x_branch_2 = GlobalMaxPool2D(data_format="channels_first")(x)
        x = concatenate([x_branch_1, x_branch_2])
        x = Dense(256, activation='relu')(x)
        x = Dropout(0.5)(x)
        x = Dense(len(config.POSSIBLE_LABELS), activation='softmax')(x)
...
class MFCCNN():
    ...
    def model_init(self, input_shape=(1, 20, 32)):
